...
function refetchIfStale(request, options) {
  // read from DB
}
...
function cleanupCache(request, cache, cacheOptions) {
  ...
  var staleOnRevalidate = cacheOptions.staleOnRevalidate;
  ...
  return idbCacheExpiration.getDb(cacheName).then(function(db) {
    ...
    return idbCacheExpiration.expireEntries(db, now, Object.assign({}, cacheOptions));
