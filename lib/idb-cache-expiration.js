...
function setTimestampForUrl(db, url, now, serveStale) {
  return new Promise(function(resolve, reject) {
...
function expireOldEntries(db, options, now) {
  ...
  return new Promise(function(resolve, reject) {
    var maxAgeMillis = options.maxAgeSeconds * 1000;
    if (options.staleOnRevalidateSeconds) {
      maxAgeMillis += options.staleOnRevalidateSeconds * 1000;
    }
    var urls = [];
...
function expireExtraEntries(db, options) {
  // Bail out early by resolving with an empty array if we're not using
  // maxEntries.
  ...
  return new Promise(function(resolve, reject) {
    ...
    index.count().onsuccess = function() {
      ...
      if (initialCount > options.maxEntries) {
        index.openCursor({
          direction: options.expireOldestFirst ? 'prev' : 'next'
          ...
          if (cursor) {
            ...
            urls.push(url);
            objectStore.delete(url);
            if (initialCount - urls.length > options.maxEntries) {
              cursor.continue();
            }
          }
        };
...
function expireEntries(db, now, options) {
  return expireOldEntries(db, options, now).then(function(oldUrls) {
    return expireExtraEntries(db, options).then(function(extraUrls) {
